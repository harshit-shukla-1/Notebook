    ...
    <div className="w-72 h-full bg-white dark:bg-zinc-950 border-r border-indigo-50 dark:border-zinc-800 flex flex-col p-4 gap-6">
      ...
      <div className="flex-1 overflow-y-auto space-y-4">
        ...
        <button
          ...
          className={cn(
            "w-full flex items-center gap-2 p-2 rounded-xl text-sm transition-all",
            activeProjectId === null 
              ? "bg-indigo-600 text-white shadow-lg shadow-indigo-200" 
              : "hover:bg-secondary/50 text-muted-foreground font-semibold"
          )}
        >
          <Inbox size={16} className={activeProjectId === null ? "text-white" : "text-indigo-500"} />
        ...
        {folders.length === 0 && (
          <div className="text-center py-8 px-4 border-2 border-dashed border-muted-foreground/10 rounded-2xl">
            <p className="text-xs text-muted-foreground">Create a folder to organize your work</p>
          </div>
        )}
      </div>
    </div>
