...
import { Folder as FolderIcon, ChevronRight, ChevronDown, Plus, Layout, Package } from 'lucide-react';
...
const Sidebar = ({ folders, projects, activeProjectId, onSelectProject, onAddFolder, onAddProject }: SidebarProps) => {
        ...
        {folders.map(folder => (
          <div key={folder.id} className="space-y-1">
            <button
              ...
              className="w-full flex items-center gap-2 p-2 hover:bg-secondary/50 rounded-xl transition-colors text-sm font-semibold group"
            >
            ...
            {expandedFolders[folder.id] && (
              <div className="ml-6 space-y-1 border-l-2 border-indigo-50 dark:border-zinc-800 pl-2">
                ...
                {projects.filter(p => p.folderId === folder.id).length === 0 && (
                  <p className="text-[10px] text-muted-foreground/50 py-2 pl-6">No projects yet</p>
                )}
              </div>
            ))}
          </div>
